...
async function collectPalettes(spritesList) {
  ...
  for (const sprite of spritesList) {
    ...
    const colorsToFind = 10;
    const minimumProportion = 0.008;
    const maximumDarkness = 0.02;
    const maximumLightness = 0.95;
    const doubleProportion = 0.5;

    // Use image pallete to be able to get percentage of colors
    const { ids, colors, amount } = imagePalette(await imagePixels(sprite.sprite), colorsToFind);

    // Filter out some colors
    let filteredColors = [];
    for (const c in colors) {
      const hex = rgbToHex(colors[c][0], colors[c][1], colors[c][2]);
      const chromaColor = chroma(hex);

      // Check darkness
      const tooDark = chromaColor.luminance() < maximumDarkness;
      const tooLight = chromaColor.luminance() > maximumLightness;

      // Check that it is signficant and not too dark
      if (amount[c] > minimumProportion && !tooDark && !tooLight) {
        filteredColors.push(hex);

        // If very signifcant, add twice
        if (amount[c] > doubleProportion) {
          filteredColors.push(hex);
        }
      }
    }
    ...
    if (filteredColors.length === 0) {
      filteredColors = colors.slice(0, 2).map(c => rgbToHex(c[0], c[1], c[2]));
    }

    finalColors[sprite.name] = filteredColors;
  }
...
function fixName(name) {
  ...
  if (name.indexOf(' Paldea') > -1) {
    name = `Paldean ${name.replace('Paldea', '')}`;
  }
  if (name.indexOf(' Hisui') > -1) {

...
router.post('/twilio', async function (req, res, next) {
  ...
  const helpColor3 = sample(colors);
  ...
  const errorResponse = `Unable to find any colors or images in that message.  Why don't you try something like "${helpColor1.name}", "${helpColor2.name}", or "${helpColor3.name}"?`;
  let message;

...
import { addLightsFromInput, addLightsFromInputOrImage } from './controller.js';
...
router.post('/', function (req, res, next) {
  ...
  try {
    const newLights = addLightsFromInput(data);
...
router.post('/twilio', async function (req, res, next) {
  ...
  // Responses
  const responses = [
    'Thanks for the colors: [[SANITIZED]].',
    'Our robots are working extra hard and found these colors: [[SANITIZED]].',
    'Wait for it... Colors!\n\nWe found these colors: [[SANITIZED]]'
  ];
  const imageResponse =
  ...
  try {
    const newLights = await addLightsFromInputOrImage(
      req.body.Body,
      req.body.MediaUrl0,

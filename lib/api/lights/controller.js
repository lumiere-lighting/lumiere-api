...
import { filter, each, uniq, flatten, sample } from 'lodash-es';
...
// Make a collection of single colors for random colors
const singleColors = filter(colors, (color) => color.colors.length === 1);
...
async function addLightsFromTextOrImage(
  ...
  const newLights = {
    ...
    colorNames: inputColors ? inputColors.colorNames : undefined,
    colors: colors,
    timestamp: new Date().getTime()
  };
...
function textToColors(input = '') {
  ...
  // Change "random" to be surrounded by commas for isolation
  let isolated = filtered.replace(/(random)/gi, ',$1,');

  // Split on commas.  As "blue, green" should be blue and green, not bluegreen
  let filteredSplit = isolated.split(',');

  // Go through each split
  let foundSplit = filteredSplit.map((separated) => {
    // Replace random
    if (separated.match(/random/i)) {
      separated = randomColor().id;
    }
    ...
    // reference.
    let searchableInput = separated.toLowerCase().trim().replace(/[^a-z0-9#]/g, '');
  ...
  return {
    colorNames: foundColors.map((color) => color.name),

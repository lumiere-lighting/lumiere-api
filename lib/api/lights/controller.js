...
// Add lights function
function addLightsFromInput(data, shouldAddSet = true) {
  // Check structure
  if (!data || !data.input) {
    debug(data);
...
async function addLightsFromInputOrImage(
  input,
  image,
  imageType,
  source,
  ...
  let filteredInput;
  if (input) {
    inputColors = inputToColors(input);
...
function inputToColors(input = '') {
  ...
  // reference.
  let searchableInput = filtered.toLowerCase().replace(/[^a-z0-9#]/g, '');
  ...
  return {
    sanitized: combinedColors.map((color) => color.name),
